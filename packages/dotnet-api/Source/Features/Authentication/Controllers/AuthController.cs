...
public class AuthController : ControllerBase
{
    ...
    /// <summary>
    /// Login with Cursor API key (for orchestration service)
    /// </summary>
    [HttpPost("cursor-login")]
    [EnableRateLimiting("AuthPolicy")]
    [ProducesResponseType<LoginWithCursorApiKeyResponse>(200)]
    [ProducesResponseType(400)]
    public async Task<ActionResult<LoginWithCursorApiKeyResponse>> CursorLogin([FromBody] CursorLoginRequest request)
    {
        var command = new LoginWithCursorApiKeyCommand(request.CursorApiKey, request.Email);
        var result = await _mediator.Send(command);

        if (result.IsSuccess)
        {
            _logger.LogInformation("User logged in with Cursor API key");
            return Ok(result.Value);
        }

        _logger.LogWarning("Cursor API key login failed: {Error}", result.Error);
        return BadRequest(new { error = result.Error });
    }
}
...
/// <summary>
/// Request model for Cursor API key login
/// </summary>
public record CursorLoginRequest(
